# TODO

## この文書の運用ルール

- **やることのみを書く。** 完了した項目は削除する（経緯は git 履歴に残る）
- Phase 別の `PLAN` / `COMPLETION` / `INVESTIGATION` は今後作らない
- 設計判断で記録を残す必要があるものだけ、独立した `DECISION` 文書にする
  （例: [`DECISION_cli_migration.md`](DECISION_cli_migration.md)）
- 各項目には**なぜ必要か**を1行添える。着手時に背景を再調査せずに済ませるため

---

## 第5段階以降の検討事項（実装しない）

先回りして実装しないこと。

### 要求の充足に関わるもの

- [ ] 書き出される前の発話が失われる件を判断する（R-12）
      発話は「終了後2.0秒の無音」が確認されて初めて文字起こしされ、
      それまではメモリ上のバッファにのみ存在する。クラッシュ・強制終了時、
      話している最中の発話（実測で最大20〜40秒ぶん）は失われる。
      `Ctrl+C` は捕捉しているため保護されている。
      ファイル入力は再実行できるが、マイク入力は音声が残らず復元できない。
      対処案（音声の並行保存 / `TAIL_MARGIN_SEC` の短縮）はいずれも
      要求外の追加または精度とのトレードオフになるため、要否から判断が要る。
      なお `requirements.md` 5.8 で「現時点では対処しない」と判断済み
- [ ] 発話区間の切り出しによる欠落を検証・対処する（R-5）
      VAD が発話と判定しなかった音声は認識処理に到達しない。小声のつぶやき、
      語尾の減衰、息継ぎ直後の立ち上がりが落ちうる。**落ちたことは出力から分からない**
      （沈黙マーカーすら入らず、単に無かったことになる）。
      `speech_pad_ms` は既定400msが効いているが、区間として検出されなかった小声には効かない。
      **音声条件で結果が大きく変わる。** Whisperへ渡した部と渡さなかった部の音量差は、
      検証音声Aで 5.3dB（余裕が小さい）、検証音声Bで 26.8dB（明確に分離）だった。
      B（語尾の減衰・小声を意図的に含む）では取りこぼしが発生しなかったが、
      A のような条件では余裕がない。録音環境に依存するため、対処の要否は継続判断。
      なお検証に使った音声は個人の発話のためリポジトリから削除済み。
      再検証する場合は同種の音声を新たに用意する

### 整理

- [ ] 文字起こしエンジンの選定
      CTranslate2 は Metal 非対応で Apple Silicon では CPU 実行になる。
      ただし実測ではリアルタイム比 0.08〜0.10 で、現状ボトルネックにはなっていない
