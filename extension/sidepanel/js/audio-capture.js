...
class AudioCapture {
    constructor(config = {}) {
        ...
        this.playbackAudio = null; // chrome.tabCapture用の音声再生
        this.isChromeTabCapture = false; // chrome.tabCaptureで取得したストリームかどうか
    ...
    /**
     * 音声キャプチャを開始（chrome.tabCaptureから - Chrome拡張専用）
     *
     * @param {Function} onChunk - チャンク生成時のコールバック
     * @param {Function} onVolumeLevel - 音量レベル更新時のコールバック
     */
        ...
        try {
            ...
            const stream = await new Promise((resolve, reject) => {
                chrome.tabCapture.capture({
                    ...
                    if (!stream) {
                        reject(new Error("タブキャプチャに失敗しました"));
                        return;
                    }
            ...
            const audioTracks = this.mediaStream.getAudioTracks();
            if (audioTracks.length === 0) {
                throw new Error("音声トラックが見つかりません。このタブには音声がないか、音声が再生されていません。");
            }

            console.log("✅ chrome.tabCaptureで音声トラックを取得:", audioTracks[0].label);

            // chrome.tabCaptureフラグを設定
            this.isChromeTabCapture = true;

            // キャプチャした音声をAudio要素で再生（ユーザーが音声を聞けるように）
            this.playbackAudio = new Audio();
            this.playbackAudio.srcObject = this.mediaStream;
            this.playbackAudio.play().catch(err => {
                console.warn("⚠️ 音声再生の自動開始に失敗（ユーザー操作が必要な場合があります）:", err);
            });

            await this._setupAudioProcessing(this.mediaStream, onChunk, onVolumeLevel);

            console.log("✅ AudioWorkletベースの音声キャプチャを開始（chrome.tabCapture）");
        } catch (error) {
            console.error("chrome.tabCaptureからの音声キャプチャ開始エラー:", error);

            // ユーザーがキャンセルした場合
            if (error.name === "NotAllowedError") {
                throw new Error("タブキャプチャがキャンセルされました");
            }
    ...
    async _setupAudioProcessing(mediaStream, onChunk, onVolumeLevel) {
        ...
        // Chrome拡張では相対パスを使用
        const processorPath = typeof chrome !== 'undefined' && chrome.runtime
    ...
    stop() {
        ...
        // Audio要素の再生を停止（chrome.tabCapture用）
        ...
        if (this.mediaStream) {
            // chrome.tabCaptureの場合はトラックを停止しない（元のタブの音声を維持）
            if (!this.isChromeTabCapture) {
