...
【未解決のリスク: 切り出しによる発話の欠落（R-5）】
この構成では、VADが発話と判定しなかった音声は認識処理に到達しない。
小声のつぶやき、語尾の減衰、息継ぎ直後の立ち上がりが落ちうる。
しかも落ちたことは出力から分からない（沈黙マーカーすら入らず、単に無かったことになる）。

現状 speech_pad_ms は既定の400msが効いており、返り値のタイムスタンプにも反映される。
これは区間の前後の切り落としには有効だが、区間として一切検出されなかった小声には効かない。

実測（sample/005-sample-a.m4a）では、採用部と破棄部の音量差が5.3dBしかなかった。
判定の余裕が小さく、取りこぼしが起きうる状態。対策は次段階の課題とする。
...
# マーカーが大量に入り、下流の LLM にとってノイズになる。
# また区間が細切れになるほど Whisper に渡る文脈が減り認識精度が落ちる。
# 以上の実用上の判断から、閾値未満の間で隔てられた区間は結合する。
#
# 注: これは要求由来の制約ではない。息継ぎにマーカーを入れること自体は
#     R-1 に反しない（実際にあった間の記録であり、推測でも修正でもない）。
#     あくまで暫定の設計判断であり、後から自由に変えてよい
